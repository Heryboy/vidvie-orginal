...
class Cart {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					...
					if ($option_query->num_rows) {
						...
						if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'box' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
				...
				$product_data[] = array(
					...
					'personalization' => $cart['personalization'],
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0,$personalization='') {
		...
		if (!$query->row['total']) {
			...
			$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "',personalization='".$personalization."', date_added = NOW()");
