...
class ControllerCatalogOption extends Controller {
	...
	protected function getForm() {
		...
		// get optionssize
		$data['optionSizes'] = array();
		$optionSizes = $this->model_catalog_option->getOptionSizes();
		// print_r($data['optionSizes']);
		foreach ($optionSizes as $optionSize) {
			$data['optionSizes'][] = array(

				'option_size_id'  => $optionSize['option_size_id'],

				'name'       => $optionSize['name'],

				'sort_order' => $optionSize['sort_order']
		...
		// if (isset($this->request->post['option_size'])) {

		// 	$option_sizes = $this->request->post['option_size'];

		// } elseif (isset($this->request->get['option_id'])) {

		// 	$option_sizes = $this->model_catalog_option->getOptionSizeDescriptions($this->request->get['option_id']);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' ||$option['type'] == 'box' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
