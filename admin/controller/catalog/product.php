...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				// 'availabel_size'   => $result['availabel_size'],
		...
		$data['entry_is_sale'] = $this->language->get('entry_is_sale');

		$data['entry_quantity'] = $this->language->get('entry_quantity');

		$data['entry_available_size'] = $this->language->get('entry_available_size');
	...
	protected function getForm() {
		...
		$data['entry_text_flag'] = $this->language->get('entry_text_flag');
		...
		$data['help_available'] = $this->language->get('help_available');
		...
		if (isset($this->request->post['text_flag'])) {

			$data['text_flag'] = $this->request->post['text_flag'];

		} elseif (!empty($product_info)) {

			$data['text_flag'] = $product_info['text_flag'];
		...
		if (isset($this->request->post['is_personalization'])) {

			$data['is_personalization'] = $this->request->post['is_personalization'];

		} elseif (!empty($product_info)) {

			$data['is_personalization'] = $product_info['is_personalization'];

		} else {

			$data['is_personalization'] = 0;
		...
		if (isset($this->request->post['available_size'])) {

			$data['available_size'] = $this->request->post['available_size'];

		} elseif (!empty($product_info)) {

			$data['available_size'] = $product_info['available_size'];

		} else {

			$data['available_size'] = 1;
		...
		if (isset($this->request->post['product_filter_available'])) {

			$filter_availables = $this->request->post['product_filter_available'];

		} elseif (isset($this->request->get['product_id'])) {

			$filter_availables = $this->model_catalog_product->getProductFilters($this->request->get['product_id']);

		} else {

			$filter_availables = array();
		...
		foreach ($filter_availables as $filter_id) {

			$filter_availabel_info = $this->model_catalog_filter->getFilterAvailable($filter_id);



			if ($filter_availabel_info) {

				$data['product_filters'][] = array(

					'filter_id' => $filter_availabel_info['filter_id'],

					'name'      => $filter_availabel_info['group'] . ' &gt; ' . $filter_availabel_info['name']
		...
		foreach ($data['product_options'] as $product_option) {
			...
			if ($product_option['type'] == 'select' || $product_option['type'] == 'box' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
		...
		foreach ($product_discounts as $product_discount) {
			...
			$data['product_discounts'][] = array(
				...
				'price_percentage'  => $product_discount['price_percentage'],
