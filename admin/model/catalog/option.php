...
class ModelCatalogOption extends Model {
	...
	public function addOption($data) {
		...
		if (isset($data['option_value'])) {
			...
			foreach ($data['option_value'] as $option_value) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "',option_size_id = '" . (int)$option_value['option_size_id'] . "',`is_instock` = '" . (isset($option_value['is_instock']) ? (int)$option_value['is_instock'] : 0) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
		...
		if (isset($data['option_value'])) {
			...
			foreach ($data['option_value'] as $option_value) {
				...
				if ($option_value['option_value_id']) {
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "',option_size_id = '" . (int)$option_value['option_size_id'] . "',`is_instock` = '" . (isset($option_value['is_instock']) ? (int)$option_value['is_instock'] : 0) . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");

				} else {

					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "',option_size_id = '" . (int)$option_value['option_size_id'] . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
	...
	public function getOptionSizeDescriptions($option_id) {
	...
	public function getOptionSizes(){
		$sql = "SELECT * FROM " . DB_PREFIX . "option_size os LEFT JOIN " . DB_PREFIX . "option_size_description osd ON (os.option_size_id = osd.option_size_id) WHERE osd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY os.sort_order, osd.name";
